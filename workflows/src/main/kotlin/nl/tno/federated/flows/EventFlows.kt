...
class NewEventFlow(
    val type : EventType,
    val previousEvent : UniqueIdentifier,
    ...
    override fun call(): SignedTransaction {
        ...
        // Retrieving previous event state
        var criteria = QueryCriteria.LinearStateQueryCriteria(uuid = listOf(previousEvent.id))
        val previousEventState = serviceHub.vaultService.queryBy<EventState>(criteria).states.single()
        ...
        // The input states are the DTs whose ID is passed as argument (i.e. those related to the event)
        criteria = QueryCriteria.LinearStateQueryCriteria(uuid = digitalTwins.map { it.id })
        val digitalTwinInputStates = serviceHub.vaultService.queryBy<DigitalTwinState>(criteria).states
